...
export const EditContact = () => {
    ...
    const idContacto = params.contactId;
    const {store} = useGlobalReducer();

    const editarcontacto = async (idContacto) => {
        let newContact = {
            name:name,
            email:email,
            phone:phone,
        ...
        try {
            const response = await fetch(API_URL_BASE + API_URL_USER+"/"+idContacto, {
                method: 'PUT',
                headers: {
    ...
    useEffect(()=>{
        const contact = store.contacts.find((contact)=>contact.id == idContacto);
            ...
            <div>
                <form className="row g-3">
                    ...
                    <div className="row mt-5 d-flex">
                        ...
                        <div className="col-1">
                            <button type="button" onClick={() => editarcontacto(idContacto)} className="btn btn-primary">Save</button>
                        </div>
                    </div>
                </form>
            </div>
